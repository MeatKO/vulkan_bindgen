...
use crate::vulkan::vk_bindgen::{
	VkDevice, VkDeviceMemory, VkFormat, VkImage, VkImageView, VkPhysicalDevice, VkQueue, VkSampler
};
use crate::vulkan::wrappers::vk_command_pool::CommandPool;
...
impl Texture<LoadedTexture>
{
	...
	pub fn process_vk(
		...
		vk_format: VkFormat
	) -> Result<Texture<VulkanTexture>, String>
	{
		unsafe
		{
			let (texture_image, texture_image_memory) = 
				create_texture_image(
					...
					physical_device,
					transfer_queue,
					&command_pool.get_command_pool_ptr(),
					&self.image, 
					vk_format
				)?;

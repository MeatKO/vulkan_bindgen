	...
	for (i, j, penetration) in penetration_pairs.iter().cloned()
	{
		...
		match axis_of_least_penetration
		{
			"x" => 
			{
				...
				// velocity_delta_vec[i].x += ((combined_momentum / mass_i)) * i_sign;
				// velocity_delta_vec[j].x += ((combined_momentum / mass_j)) * j_sign;
			},
			"y" => 
			{
				...
				// velocity_delta_vec[i].y += ((combined_momentum / mass_i)) * i_sign;
				// velocity_delta_vec[j].y += ((combined_momentum / mass_j)) * j_sign;
			},
			"z" => 
			{
				...
				// velocity_delta_vec[i].z += ((combined_momentum / mass_i)) * i_sign;
				// velocity_delta_vec[j].z += ((combined_momentum / mass_j)) * j_sign;
			},
...
// 	let mut new_translations = aabb_vector.iter().map(|aabb| aabb.velocity).collect::<Vec<Vec3>>();
...
// 			// works only for cubes of equal size me thinks... ferris_clueless
// 			let other_center_dir = aabb_vector[j].translation - aabb_vector[i].translation;

// 			let other_speed = aabb_vector[j].velocity.len();
// 			let friction_factor = other_speed.sqrt().min(other_speed);// / 100.0f32;
// 			let friction_velocity_influence = aabb_vector[j].velocity.normalize() * friction_factor;
...
// 					"x" => 
// 					{
// 						new_velocities[i].x = -new_velocities[i].x * 0.95f32;
// 						new_velocities[i] += Vec3{x: 0.0f32, ..friction_velocity_influence};
// 						new_translations[i].x -= other_center_dir.x.signum() * penetration.x;
// 					},
// 					"y" => 
// 					{
// 						new_velocities[i].y = -new_velocities[i].y * 0.95f32;
// 						new_velocities[i] += Vec3{y: 0.0f32, ..friction_velocity_influence};
// 						new_translations[i].y -= other_center_dir.y.signum() * penetration.y;
// 					},
// 					"z" => 
// 					{ 
// 						new_velocities[i].z = -new_velocities[i].z * 0.95f32;
// 						new_velocities[i] += Vec3{z: 0.0f32, ..friction_velocity_influence};
// 						new_translations[i].z -= other_center_dir.z.signum() * penetration.z;
// 					},
// 					_ => {},
// 				}
...
// 	for (index, aabb) in aabb_vector.iter_mut().enumerate()
// 	{
// 		aabb.translation += new_translations[index];
// 	}

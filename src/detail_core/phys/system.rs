...
pub fn physics_system_2()
{
	...
	match raycast_object.state
	{
		...
		{
			if !aabb_vector[index].is_static
			{
		...
		RaycastObjectState::Picked((index, length, obj_relative_hit)) =>
		{
			if !aabb_vector[index].is_static
			{
				aabb_vector[index].translation = vk_handle.camera.get_position() + (vk_handle.camera.get_front() * length) + obj_relative_hit;
				aabb_vector[index].velocity = Vec3::new(0.0f32);
			}
		}
		_ => {}
	}
	...
	for aabb in aabb_vector.iter_mut()
	{
		...
		aabb.velocity += gravity * (delta_time.last_delta_time / 50000.0f32);
	}

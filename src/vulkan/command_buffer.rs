use decs2::query::Query;
use decs2::query::Read;
use decs2::query::Write;
...
pub unsafe fn record_command_buffer(
	...
	// model_vec: &Vec<(&mut VulkanModelComponent, &UniformBufferComponent, &AABB)>,
	model_vec: &Query<'_, (Write<VulkanModelComponent>, Read<UniformBufferComponent>, Read<AABB>)>,
...
)
{
	...
	{
		let model_asset_ref = model_assets_map.get(&model_component.model_asset_name).unwrap();

...
pub unsafe fn record_command_buffer_wireframe_ref(
	...
	image_index: u32,
	// phys_boxes: &Vec<&AABB>,
	phys_boxes: &Query::<Read<AABB>>,
	default_aabb_mesh_data: &VulkanMeshData,
)
{
	...
	for phys_box in phys_boxes.iter()
	{
		let vulkan_data = 
			match &phys_box.aabb_uniform_data
			{
				Some(vd) => vd,
				None => {continue}
			};
		...
		let vertex_buffers: Vec<VkBuffer> = 
			vec![
				default_aabb_mesh_data.vertex_buffer,
			];
		...
		vkCmdBindIndexBuffer(
			current_command_buffer,
			default_aabb_mesh_data.index_buffer, 
			0,
		...
		vkCmdDrawIndexed(
			current_command_buffer,
			default_aabb_mesh_data.index_count, 
			1, 
			0, 
			0, 
			0
